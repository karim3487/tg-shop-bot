...
После запуска можно заполнить базу тестовыми данными: категориями, товарами с изображениями, клиентами, заказами и FAQ.

```bash
make seed
```

Команда идемпотентна — повторный запуск не дублирует записи. Чтобы очистить данные перед заполнением:

```bash
make seed-clear
```

> **Примечание:** Если суперпользователь `admin` ещё не создан, команда создаст его автоматически с паролем `admin`.
